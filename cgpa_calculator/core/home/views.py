...
def gpa_calculator(request):
    if request.method == 'POST':
        # Retrieve grade values from the form
        grades = []
        for i in range(1, 9):  # Assuming 8 semesters
            grade = request.POST.get(f'grade{i}')
            if grade:
                grades.append(float(grade))  # Convert to float instead of int
        # Calculate GPA based on grades
        if grades:
            total_marks = sum(grades)
            total_semesters = len(grades)
            gpa = total_marks / total_semesters
            # You can round the GPA to 2 decimal places if needed
            # gpa = round(gpa, 2)
            return render(request, 'gpa_calculator.html', {'gpa': gpa})
        else:
            return render(request, 'gpa_calculator.html', {'error_message': 'No grades provided'})
    else:
